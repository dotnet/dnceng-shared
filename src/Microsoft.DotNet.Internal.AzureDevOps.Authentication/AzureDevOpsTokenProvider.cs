...
namespace Microsoft.DotNet.Internal.AzureDevOps.Authentication;
...
public class AzureDevOpsTokenProvider : IAzureDevOpsTokenProvider
{
    ...
    private static readonly Regex AccountNameRegex = new(@"^https://(?:([^@/]+@)?dev\.azure\.com/(?<account>[a-zA-Z0-9]+)/|(?<account>[a-zA-Z0-9]+)\.visualstudio\.com/)");
